...
DOMContentLoaded.addEventOrExecute(() => {
	...
	{% if template == 'home' %}
        ...
        window.swiperLoader('.js-home-slider-categories', {
            threshold: 5,
            watchSlideProgress: true,
            watchSlidesVisibility: true,
            slideVisibleClass: 'js-swiper-slide-visible',
        {% if settings.slider_categories | length > 8 %}
            loop: true,
        {% endif %}
            navigation: {
                nextEl: '.js-swiper-home-categories-next',
                prevEl: '.js-swiper-home-categories-prev',
            },
            slidesPerView: 2.5,
            spaceBetween: 15,
            breakpoints: {
        ...
        {% set has_extra1_products_slider = sections.extra1.products and settings.extra1_products_format != 'grid' %}
        {% set has_extra2_products_slider = sections.extra2.products and settings.extra2_products_format != 'grid' %}

        {% if has_featured_products_slider or has_new_products_slider or has_sale_products_slider or has_extra1_products_slider or has_extra2_products_slider %}
            ...
            {% if has_extra1_products_slider %}

                window.swiperLoader('.js-swiper-extra1', {
                    lazy: lazyVal,
                    watchOverflow: watchOverflowVal,
                    centerInsufficientSlides: centerInsufficientSlidesVal,
                    threshold: 5,
                    ...
                    watchSlidesVisibility: true,
                    slideVisibleClass: 'js-swiper-slide-visible',
                {% if sections.extra1.products | length > 3 %}
                    loop: true,
                {% endif %}
                    navigation: {
                        nextEl: '.js-swiper-extra1-next',
                        prevEl: '.js-swiper-extra1-prev',
                    },
                    slidesPerView: slidesPerViewMobileVal,
                    breakpoints: {
                        768: {
            ...
            {% if has_extra2_products_slider %}

                window.swiperLoader('.js-swiper-extra2', {
                    lazy: lazyVal,
                    watchOverflow: watchOverflowVal,
                    centerInsufficientSlides: centerInsufficientSlidesVal,
                    threshold: 5,
                    ...
                    watchSlidesVisibility: true,
                    slideVisibleClass: 'js-swiper-slide-visible',
                {% if sections.extra2.products | length > 3 %}
                    loop: true,
                {% endif %}
                    navigation: {
                        nextEl: '.js-swiper-extra2-next',
                        prevEl: '.js-swiper-extra2-prev',
                    },
                    slidesPerView: slidesPerViewMobileVal,
                    breakpoints: {
                        768: {
	...
	function changeVariant(variant) {
	    ...
	    if (variant.price_short){
            ...
            var price_pix_discount_raw = parseInt( {{ product.maxPaymentDiscount.value }} )/100;
            var_price_pix = ((variant_price_raw/100)*(1 - price_pix_discount_raw)).toFixed(2);

	        parent.find('.js-price-display').text(variant.price_short).show();
            jQueryNuvem('.js-price-pix').text(var_price_pix.replace('.',','));
	    ...
	    if (!variant.available){
	        button.find('span').html('{{ "Sin stock" | translate }}');
	        button.addClass('nostock');
	        button.attr('disabled', 'disabled');
	        $product_shipping_calculator.hide();
	    } else if (variant.contact) {
	        button.find('span').html('{{ "Consultar precio" | translate }}');
	        button.addClass('contact');
	        button.removeAttr('disabled');
	        $product_shipping_calculator.hide();
	    } else {
	        button.find('span').html('{{ "Agregar al carrito" | translate }}');
