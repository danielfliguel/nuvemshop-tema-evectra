...
{% endif %}
	<div class="{% if mobile %}js-home-mobile-slider{% else %}js-home-main-slider{% endif %}-visibility {% if has_main_slider and has_mobile_slider %}{% if mobile %}d-md-none{% else %}d-none d-md-block{% endif %}{% elseif not settings.toggle_slider_mobile and mobile %}hidden{% endif %} {% if not settings.slider_full %}{% endif %} mb-4">
			...
			<div class="row">
				<div class="col section-slider{% if settings.slider_full %} p-0{% endif %}{% if has_auto_height_slider %} section-slider-auto{% endif %}">
					<div class="js-home-slider{% if mobile %}-mobile{% endif %} nube-slider-home swiper-container{% if settings.theme_rounded and not settings.slider_full %} box-rounded{% endif %} swiper-container-horizontal">
						...
						<div class="js-swiper-home-control bg-text js-swiper-home-prev{% if mobile %}-mobile{% endif %} swiper-button-prev d-none d-md-flex align-items-center justify-content-center svg-icon-text{% if settings.icons_solid %} svg-solid{% endif %}">
							<svg class="icon-inline icon-2x mr-1 icon-flip-horizontal icon-primary"><use xlink:href="#chevron"/></svg>
						</div>
						<div class="js-swiper-home-control bg-text js-swiper-home-next{% if mobile %}-mobile{% endif %} swiper-button-next d-none d-md-flex align-items-center justify-content-center svg-icon-text{% if settings.icons_solid %} svg-solid{% endif %}">
							<svg class="icon-inline icon-2x ml-1 icon-primary"><use xlink:href="#chevron"/></svg>
						</div>
					</div>
				</div>
			</div>
