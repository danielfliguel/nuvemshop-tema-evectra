...
<header class="js-head-main head-main head-{{ settings.head_background }} {% if settings.head_fix %}head-fix{% endif %} transition-soft" data-store="head">
    ...
    {% set has_ad_bar = settings.ad_bar %}
    {% set had_ad_bar_services = settings.ad_bar_services %}
    {% set show_topbar =  has_ad_bar or has_social_network or has_languages %}
    {% if show_topbar %}
        <section class="js-topbar section-topbar py-2 d-none d-lg-block bg-text {% if not has_ad_bar %}d-none d-md-block{% endif %}">
            <div class="container">
                <div class="row">
                    <div class="col text-left d-none d-md-flex align-items-center">
	...
	<section class="header-items">
        <div class="container {% if settings.head_utility == 'searchbox' %}pb-3 pb-md-0{% endif %}">
            <div class="d-lg-flex d-none justify-content-between gap-between-lg align-items-center header-contact-info mb-3">
                <div class="top-menu">
                    {% if settings.header_menu_above_searchbar_show and settings.header_menu_above_searchbar | length > 0 %}
                            {% snipplet "navigation/navigation-head-above-searchbar.tpl" %}    
                    {% endif %}
                </div>
                <div class="header-contact-info d-flex gap-between-lg">                   
                    {% if store.email %}
                        <a href="mailto:{{ store.email }}">{{ store.email }}</a>
                    {% endif %}
                    {% if store.whatsapp %}
                        <a target="_blank" href="{{ store.whatsapp }}">
                            {{ 'Ayuda' | translate }}
                        </a>
                    {% endif %}
                    <a href="/account/orders">{{ 'Mis pedidos' | translate }}</a>
                </div>
            </div>
            <div class="{% if not settings.head_fix %}js-nav-logo-bar{% endif %} row no-gutters align-items-center header-elements mb-3">
                ...
                {% if settings.head_utility == 'searchbox' or settings.head_utility == 'icons' or (settings.logo_position_mobile == 'left' and settings.head_utility == 'categories') %}
                    <div class="col-auto {% if settings.logo_position_mobile == 'left' %}order-last ml-2{% else %}text-left{% endif %} d-flex align-items-center d-lg-none">
                ...
                <div class="{% if settings.logo_position_desktop == 'center' %}{% if settings.icons_size_desktop == 'big' %}col-md-6 col-lg-6{% else %}col-md-4 col-lg-4{% endif %}{% else %}col-lg-2{% endif %} {% if settings.logo_position_mobile == 'left' %}col text-left{% else %}col text-center{% endif %} {% if settings.logo_position_desktop == 'center' %}text-md-center {% if settings.icons_size_desktop == 'small' %}offset-md-1{% endif %} order-md-2{% else %}text-lg-left{% endif %}">
                ...
                <div class="{% if settings.head_utility == 'searchbox' %}col-12 mb-3 mb-lg-0 order-last order-lg-0 d-lg-flex flex-lg-column justify-content-lg-between{% else %}col-6 d-none d-md-block{% endif %} text-center {% if settings.logo_position_desktop == 'center' %}col-md-3 col-lg-3 order-md-1{% elseif settings.icons_size_desktop == 'small' and settings.logo_position_desktop == 'left' %}col-md-6 col-lg-5{% else %}col-lg-5{% endif %}">
                ...
                <div class="col-auto {% if settings.logo_position_mobile == 'left' %}order-3{% elseif settings.logo_position_mobile == 'center' and settings.head_utility == 'icons' %}order-last{% endif %} {% if settings.icons_size_desktop == 'small' %}col-md-4 col-lg-4{% else %}col-lg-5 d-flex align-items-center{% endif %} {% if settings.logo_position_desktop == 'center' %}order-md-3{% endif %}">
                    {% if settings.head_fix and settings.ajax_cart %}
                        <div class="d-none d-md-block">
                            {% include "snipplets/notification.tpl" with {add_to_cart: true} %}
                        </div>
                    {% endif %}
                    {% snipplet "header/header-utilities-custom.tpl" %}
                </div>
            ...
            <div class="row align-items-center {% if settings.head_utility == 'searchbox' %}d-none d-md-block{% endif %}">
                ...
                {% endif %}
                {% if not settings.home_side_navigation_menu %}
